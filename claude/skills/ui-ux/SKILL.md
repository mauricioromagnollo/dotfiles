---
name: ui-ux
description: Projetar, construir e revisar interfaces com foco em usabilidade, experiência e design visual — fundamentado em Norman (affordances, feedback, modelo mental), Krug (não me faça pensar), Gestalt, heurísticas de Nielsen, Laws of UX, acessibilidade (WCAG) e padrões modernos de UI. Use ao criar ou alterar qualquer tela, componente, formulário, fluxo, navegação, landing page ou estado de UI (loading/vazio/erro); ao escolher layout, tipografia, cor, espaçamento ou hierarquia; ao decidir copy de botão/label/mensagem de erro; ao revisar uma interface. Dispare também em pedidos como "melhore essa tela", "isso está confuso", "está feio", "a UX está ruim", "esse formulário é chato de preencher", "revise a usabilidade", "isso é acessível?", "qual o melhor padrão para X", "onde coloco esse botão", "como deixo isso mais intuitivo", "o usuário não acha o X", "faça a home", "design desse card/modal/menu" — mesmo que o usuário não cite "UX", "UI", "usabilidade" ou "acessibilidade" pelo nome. Dispare ao revisar UI com muitos cliques, hierarquia confusa, texto que ninguém lê, contraste baixo, alvo de toque pequeno ou fluxo que exige o usuário pensar. Também para justificar NÃO redesenhar e seguir uma convenção existente.
---
